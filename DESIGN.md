...
Tempo Time uses the Tauri utility template, as explicitly requested. Keep the coral tap circle, cream/dark input area and ink note panel. Only input and channel lists, a selected-channel level meter and a compact icon-only Listen/Stop control sit above the circle. Keep the control hitbox and layout fixed across connection and error states. Tapping enters Manual automatically, indicated by a small status label, and returns to Audio 30 seconds after the last tap. No mode selector or BPM field. The right panel contains only the six Android note rows (whole through thirty-second) and milliseconds. No pre-delay hero, selected rows, copy controls, rhythm settings, detector options or explanatory text. Each audio click is a quarter note. Channel selection is a list, never a typed number. The app icon places its original coral mark on a white macOS rounded-square background.
